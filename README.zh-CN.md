...
| 类别 | 形态 |
|---|---|
| **资金费 / 基差套利** | 永续对现货或永续对永续，围绕结算时刻，对冲持有 |
| **跨平台套利** | 同一标的在两个平台、两个账户，一个策略 |
| **做市** | 按库存和波动率管理的双边报价；含 DEX 上的激励区间报价 |
| **统计套利 / 配对交易** | 价差或 z-score monitor 驱动的多腿对冲头寸 |
| **趋势跟踪 / 均值回归** | 指标驱动的方向性策略，适用于任何市场 |
| **网格 / 定投** | 按计划或按价位触发的累积与派发 |
| **跟单** | 按比例镜像目标账户或钱包，带仓位上限 |
| **链上收益** | 基于钱包密钥的账户，接入借贷、LP、收益代币化协议 |
| **链上套利** | DEX 之间、DEX 与 CEX 之间的价差，由钱包账户执行 |
| **撸毛** | 多钱包按计划与协议交互，每个钱包一个账户 |
| **打新** | 监控上币与代币发行，在事件发生时按仓位上限进入 |
| **Meme 交易** | 链上快速动量，硬止损与仓位限制 |
| **新闻 / 社交信号** | 新闻源与社交帖子（X、Telegram）的 monitor，LLM 分类后带风控交易 |
| **AI 信号** | 结构化输出的 LLM 推理作为输入之一，风控写在代码里 |
